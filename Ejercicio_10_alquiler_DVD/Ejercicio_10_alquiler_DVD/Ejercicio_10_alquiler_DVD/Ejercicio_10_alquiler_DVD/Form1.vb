        ...
        If opcion <> "" Then
            ...
            If boolDevol Then
                If diasTranscurridos > coleccionDVD(opcion).GetSet_DiasAlquilerDvd() Then
                    ...
                    DevolverDVD.Show()
                    MsgBox("DVD devuelto")
                    MsgBox("Ha superado plazo en " & diasTranscurridos & " dias. Debe abonar " & diasTranscurridos & "€")
                Else
                    Me.Hide()
                    DevolverDVD.Show()
    ...
    Private Sub CONSULTACLIENTESToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles CONSULTACLIENTESToolStripMenuItem.Click
        ...
        If opcion <> "" Then
            If coleccionClientes.Contains(opcion) Then
                MsgBox("Codigo cliente: " & coleccionClientes(opcion).GetSet_codigoCli() & vbNewLine &
                       "Direccion cliente: " & coleccionClientes(opcion).GetSet_direccionCli() & vbNewLine &
                       "Telefono cliente: " & coleccionClientes(opcion).GetSet_telefonoCli() & vbNewLine &
                       "Email cliente: " & coleccionClientes(opcion).GetSet_emailCli() & vbNewLine &
                       "Alquieler del cliente: " & coleccionClientes(opcion).GetSet_alquileresCli())
            Else
